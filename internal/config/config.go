...
func Load() Config {
	...
	if databaseURL == "" {
		databaseURL = "postgres://" + getEnv("POSTGRES_USER", "admin") + ":" + getEnv("POSTGRES_PASSWORD", "password") + "@" + getEnv("POSTGRES_HOST", "db") + ":" + getEnv("POSTGRES_PORT", "5432") + "/" + getEnv("POSTGRES_DB", "imageboard")
	}
	...
	return Config{
		...
		MaxBodySize:         int64(getEnvAsInt("MAX_BODY_SIZE", 1048576)),
